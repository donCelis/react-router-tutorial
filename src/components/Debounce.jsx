...
const User = ({ user }) => {
  return (
    <div key={user.id} className='flex flex-col items-center m-4'>
      ...
      <span className='text-lg text-gray-400 font-bold'>{user.id}</span>
    </div>
  )
}
...
export function Debounce () {
  ...
  const searchUsers = async () => {
    const newUsersResponse = await axios.get(
      `https://dummyjson.com/users/search?q=${value}&limit=10`
    )
  ...
  return (
    <div className='h-full flex flex-col w-full items-center p-12'>
      <form className='flex justify-center w-1/2 mb-12'>
        <SearchBarInput defaultValue={value} onChange={handleSearchInputChange} />
      </form>
